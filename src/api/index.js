...
class Axios {
  constructor () {
    ...
    _axios.interceptors.request.use(this.handleSuccessRequest, this.handleErrorRequest)
    _axios.interceptors.response.use(this.handleSuccessResponse, this.handleErrorResponse)
    this._axios = _axios
  }
  ...
  handleSuccessRequest = req => {
    ...
    console.groupCollapsed(`API request: ${req.url}`)
  ...
  handleErrorResponse = error => {
    ...
    if (errorData.description) errorMsg += ` and description: "${errorData.description}"`
  ...
  async post (url, data) {
    try {
      const resp = await this._axios.post(url, data)
      return resp
    } catch (error) {
      return error
    }
  ...
  async patch (url, data) {
    try {
      const resp = await this._axios.patch(url, data)
      return resp
    } catch (error) {
      return error
    }
  ...
  async put (url, data) {
    try {
      const resp = await this._axios.put(url, data)
      return resp
    } catch (error) {
      return error
    }
